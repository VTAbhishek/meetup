 ...
 *   GET               -> { tables: [{id,category,table_no,seats,note,image_url,is_active,sort_order}] }
 *   POST (multipart)  -> add one table: category, table_no, seats, note?, image?
 *   POST ?id= (JSON)  -> update a table: { is_active?, category?, table_no?, seats?, note? }
 *                        (the on/off switch posts { is_active })
...
if ($method === 'POST' && isset($_GET['id'])) {
    $id  = (int) $_GET['id'];
    $own = $pdo->prepare('SELECT id FROM company_tables WHERE id = ? AND company_id = ?');
    $own->execute([$id, $cid]);
    if (!$own->fetchColumn()) json_error('Table not found', 404);
...
if ($method === 'POST') {
    ...
    $pdo->prepare(
        'INSERT INTO company_tables (company_id, category, table_no, seats, note, image, sort_order)
         VALUES (?, ?, ?, ?, ?, ?, ?)'
    )->execute([$cid, $category, $tableNo, $seats, $note ?: null, $image, $order + 1]);
